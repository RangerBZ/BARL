...
class Actor(nn.Module):
    ...
    @torch.no_grad()
    def generate(self, input_ids: torch.Tensor, is_mc=False, **kwargs) -> Union[
    ...
    def forward(
        ...
        if not self.packing_samples:
            ...
            output_logits = output["logits"][:, -num_actions:, :]
        else:
            ...
            for num_action, seq_len in zip(num_actions, packed_seq_lens):
                ...
                output_logits.append(output["logits"][:, start:end, :])
                offset += seq_len
            action_log_probs = torch.cat(action_log_probs, dim=1)
            output_logits = torch.cat(output_logits, dim=1)

        if return_output:
            return (action_log_probs, output_logits)
        else:
