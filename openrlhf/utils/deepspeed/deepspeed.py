...
class DeepspeedStrategy(ABC):
    ...
    def create_optimizer(self, model, var_lr, **kwargs) -> Optimizer:
        ...
        optim_params = get_optimizer_grouped_parameters(model, kwargs["weight_decay"], kwargs["lr"], var_lr)
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            if isinstance(model_to_save, PeftModel):
                layer_params = {k: v for k, v in model_to_save.state_dict().items() if "base_model.model.score.base_layer.weight" in k}
                torch.save(layer_params, os.path.join(output_dir, 'score_base.pth'))
